...
if __name__ == "__main__":
    # 사용자로부터 특정 일자 입력 받기
    sale_date_input = input("Enter the sale date (YYYYMMDD): ")

    # 입력한 날짜 검증
    sale_date = validate_date(sale_date_input)
        ...
        try:
            with connection.cursor() as cursor:
                ...
                for sp_code in sp_codes:
                    fetch_and_store_product_sales_data(sp_code[0], sale_date_str)
        finally:
            connection.close()
            log_message(f"Data fetching and processing completed for {sale_date_str}")
    else:
