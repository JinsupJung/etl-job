...
log_filename = f"/home/nolboo/etl-job/log/sales_data_log_date_{today}.txt"
logging.basicConfig(
    level=logging.INFO,
...
if __name__ == "__main__":
    # 지정된 일자를 입력받습니다.
    sale_date = input("Enter the sale date (YYYYMMDD): ")

    logging.info(f"Starting data fetching and processing for {sale_date}")
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for sp_code in sp_codes:
                fetch_and_process_sales_data(sp_code[0], sale_date)
    finally:
        connection.close()
